...
-fcheck-new -fsized-deallocation -fstack-protector -fstrict-overflow -fno-omit-frame-pointer -Wstack-usage=8192 -pie -fPIE -Werror=vla \
...
SRC_LIST = ./third-party/List/src/
OBJ_LIST = ./third-party/List/obj/

$(OBJ_LIST)%.o : $(SRC_LIST)%.cpp
	@mkdir -p $(@D)
	@$(CC) $(IFLAGS) $(CFLAGS) -c $< -o $@

SRC    = $(wildcard $(SRC_FOLDER)*.cpp)
OBJ    = $(patsubst $(SRC_FOLDER)%.cpp, $(OBJ_FOLDER)%.o, $(SRC))
SRC_C  = $(wildcard $(SRC_LIST)*.cpp)
OBJ   += $(patsubst $(SRC_LIST)%.cpp, $(OBJ_LIST)%.o, $(SRC_C))
